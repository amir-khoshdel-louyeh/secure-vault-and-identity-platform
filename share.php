...
class Share {
     ...
     * 
     * @param int $userId شناسه کاربر مالک
     * @param string $itemType نوع آیتم ('file' یا 'note')
     * @param int $itemId شناسه فایل یا نوت
     * @param int $expireHours مدت زمان اعتبار لینک به ساعت (مثلاً 24)
     * @param int $maxUses حداکثر تعداد دفعات استفاده (مثلاً 1)
     */
    public function createShareToken(int $userId, string $itemType, int $itemId, int $expireHours = 24, int $maxUses = 1): array {
        ...
        // ۲. تولید توکن ۶۴ کاراکتری کرپیتوگرافیک امن
    ...
    /**
     * دسترسی عمومی به فایل یا نوت اشتراک‌گذاری‌شده از طریق توکن
     */
    public function accessSharedItem(string $token): void {
        ...
        if ($share['max_uses'] > 0 && $share['uses_count'] >= $share['max_uses']) {
            http_response_code(410);
            die("این لینک یک‌بارمصرف بوده و قبلا استفاده شده است.");
        }
    ...
    private function serveSharedNote(int $noteId): void {
        ...
        // خروجی امن HTML جهت جلوگیری از XSS
